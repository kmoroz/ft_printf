...
/*                                                   +#+                      */
/*   Created: 2021/02/17 13:13:16 by ksmorozo      #+#    #+#                 */
/*   Updated: 2021/02/17 13:36:34 by ksmorozo      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int			print_u(va_list *arguments, t_recipe recipe)
{
	...
	num = deal_with_length_unsigned(arguments, recipe);
	check_recipe(&recipe, num);
	num_length = count_num_length_unsigned(num, 10);
	if (recipe.null_precision && !num)
		num_length = 0;
	padding_len = count_padding_len(recipe, num_length, num);
	converted_number = ft_itoa_base_unsigned(num, 10, "0123456789");
	if (!converted_number)
		return (-1);
